...
public class ParallelDataSet<T> extends BaseDataSet<T> {
    ...
    public Flowable<PartialResult<JsonList<ControlMessage.Status>>> manage(ControlMessage message) {
        ...
        List<Flowable<PartialResult<JsonList<ControlMessage.Status>>>> obs =
                new ArrayList<Flowable<PartialResult<JsonList<ControlMessage.Status>>>>();
        final int mySize = this.size();
        // Run over each child separately
        for (int i = 0; i < mySize; i++) {
            IDataSet<T> child = this.children.get(i);
            Flowable<PartialResult<JsonList<ControlMessage.Status>>> sk = child.manage(message);
        ...
        final Flowable<PartialResult<JsonList<ControlMessage.Status>>> pr = executed.map
                (l -> new PartialResult<JsonList<ControlMessage.Status>>(0, l));
        obs.add(pr);

        Flowable<PartialResult<JsonList<ControlMessage.Status>>> merged = Flowable.merge(obs);
